...
def do_96_heatmap_photophysics_parameters(exp, list_of_poca_files, list_of_frame_csv, list_of_int_csv, list_of_sigma_csv, 
    ...
    for f in range(len(list_of_poca_files)):
        ...
        well_data['_phot_per_loc'] = int(stats(photon_calculation(pre_process_single_intensity(list_of_int_csv[f], on_filter=drop_one_event, beads=drop_beads))))
    ...
    sns.heatmap(tab_n, annot=tab_orig, fmt=".1f", cmap="seismic", vmin=0, vmax=1)
